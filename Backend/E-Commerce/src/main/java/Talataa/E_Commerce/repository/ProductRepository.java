...
public interface ProductRepository {
    ...
    @Select(" select product.id as idProducto, product.nombre as productoNombre," +
            ...
            " where product.estado = 'A' and inventarioProducto.estado = 'A' and categoria.estado = 'A'" +
            " and inventarioProducto.cantidadDisponible>0 order by product.nombre")
    List<ProductosResponse> productosDisponibles();
    ...
    Integer guardarProducto(ProductoRequest producto);

    @Insert("INSERT INTO PRODUCTOS_CATEGORIAS (idProducto, idCategoriaProducto) VALUES (#{idProducto}, #{idCategoriaProducto})")
    Integer guardarProductoCategoria(@Param("idProducto") Integer idProducto, @Param("idCategoriaProducto") Integer idCategoriaProducto);
    ...
    @Update("UPDATE PRODUCTOS SET " +
            ...
            "WHERE id = #{idProducto}")
    Integer inactivarProducto(@Param("idProducto") Integer idProducto);
    ...
    @Update("UPDATE PRODUCTOS SET " +
            ...
            "update_at = CURRENT_TIMESTAMP " +
            "WHERE id = #{id} ")
    Integer actualizarProducto(ProductoRequest producto);

    @Update("UPDATE PRODUCTOS_CATEGORIAS SET " +
            "idCategoriaProducto = #{idCategoriaProducto} " +
            "WHERE idProducto = #{idProducto}")
    Integer actualizarProductoCategoria(@Param("idProducto") Integer idProducto, @Param("idCategoriaProducto") Integer idCategoriaProducto);
}
